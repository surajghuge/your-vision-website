...
const AgentDetailModal = ({ agent, onClose }: AgentDetailModalProps) => {
  ...
  const hasInput = Boolean(agent.inputDetails.trim());
  const hasOutput = Boolean(agent.outputDetails.trim());
  const hasExample = Boolean(agent.example.trim());
  const hasUseCases = agent.useCases.length > 0;
  const hasPointOfContact = Boolean(agent.pointOfContact.trim());
